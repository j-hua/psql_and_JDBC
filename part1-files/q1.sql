...
INSERT INTO q1 SELECT assignment_id, 100*AVG(percentage),SUM(case when percentage>=.80 then 1 else 0 end) above80,SUM(case when percentage>=0.60 and percentage<0.80 then 1 else 0 end) above60,SUM(case when percentage>=0.50 and percentage<0.60 then 1 else 0 end) above50,SUM(case when percentage<0.5 then 1 else 0 end) failed
FROM totalmark
